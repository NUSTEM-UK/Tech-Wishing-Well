        ...
        try:
            while True:
                ...
                with Timer() as t_capture:
                    ...
                    with stream.lock:
                        # Just grab the first image, whatever it is
                        stream.seek(0)
                        capture_frame = Image.open(stream) 
                    ...
                    # ...which merrily continues on a background thread. We hope.

                    # So now we can process the image
                    frameyuv = capture_frame.convert("YCbCr")
                    frameyuv_array = np.array(frameyuv)
                    y = frameyuv_array[0:width, 0:height, 0] # Get just the y component as a numpy array
                print "=> Capture & conversion done in %d s" % t_capture
